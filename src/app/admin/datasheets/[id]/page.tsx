...
function normalizeColumns(raw: unknown): Column[] {
  ...
  for (const item of raw) {
    ...
    const width =
      typeof obj.width === 'number' && Number.isFinite(obj.width) ? obj.width : undefined;
    columns.push({
      id: newId('col'),
      header,
      type,
...
function normalizeRows(raw: unknown, columnCount: number): Row[] {
  ...
  for (const item of raw) {
    ...
    const rawCells = Array.isArray(obj.cells)
      ? obj.cells.filter((c): c is string => typeof c === 'string')
      : [];
...
function normalizeCurves(
  raw: unknown,
): { ok: true; curves: Curve[] } | { ok: false; error: string } {
  if (!Array.isArray(raw)) {
...
export default function DatasheetEditorPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const handleImport = () => {
    try {
      ...
      const cover =
        src.cover !== null && src.cover !== undefined ? normalizeCover(src.cover) : undefined;
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            onChange={(e) =>
              patch((d) => ({
                ...d,
                meta: { ...d.meta, partNumber: e.target.value, title: e.target.value },
              }))
            }
      ...
      <div className="mx-auto flex w-full max-w-[1600px] flex-1 flex-col lg:flex-row">
        {viewMode !== 'preview' && (
          <aside className="w-full shrink-0 border-b border-gray-200 bg-white lg:sticky lg:top-[52px] lg:h-[calc(100vh-52px)] lg:w-[400px] lg:overflow-y-auto lg:border-b-0 lg:border-r">
            ...
            <div
              role="tablist"
              className="sticky top-0 z-10 grid grid-cols-5 gap-1 border-b border-gray-100 bg-white p-2"
            >
              {TABS.map((t) => (
                <button
                  key={t.key}
            ...
            <div className="p-3">
              ...
              {sidebarTab === 'cover' && (
                <CoverEditor
                  cover={doc.cover}
                  onChange={(cover) => patch((d) => ({ ...d, cover }))}
                />
              )}
              {sidebarTab === 'sections' && (
                <SectionsEditor
