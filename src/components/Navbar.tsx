...
const aboutData: Record<string, Record<string, string[]>> = {
  ...
  'Corporate Responsibility': {
    'Environment': ['Overview', 'Sustainability Priorities', 'Our Approach', 'Climate Change', 'GHG Emissions', 'Water Management', 'Waste Management', 'Sustainable Products and Services', 'Opportunities in Clean Tech'],
    'Social': ['Overview', 'Human Capital Management', 'Health and Safety', 'Inclusive Workplace', 'Community', 'Supply Chain Management'],
    'Governance': ['Overview', 'Oversight Structure', 'Board Composition and Role', 'Risk Management', 'Cybersecurity and Data Privacy'],
    'Ethics & Compliance': [],
    'TCFD Index': [],
  },
  'Newsroom': {},
...
export default function Navbar() {
  ...
  useEffect(() => {
    if (openMenu !== 'products') { setProductCat(null); setProductFam(null) }
    if (openMenu !== 'applications') { setAppCat(null); setAppSub(null) }
    if (openMenu !== 'about') { setAboutCat(null); setAboutSub(null) }
    if (openMenu !== 'design-resources') { setDrCat(null) }
  }, [openMenu])
      ...
      <nav className="max-w-[1280px] mx-auto flex items-center justify-between px-6 py-4 relative">
        ...
        <div className="hidden md:flex items-center gap-0">
          ...
          <div className="ml-2 flex items-center">
            {searchOpen ? (
              <div className="flex items-center bg-gray-100 rounded-lg border border-gray-200 overflow-hidden">
                <input autoFocus type="text" placeholder={t.nav.search} className="bg-transparent text-sm text-gray-900 px-3 py-2 w-48 outline-none placeholder-gray-400" />
                <button type="button" onClick={() => setSearchOpen(false)} aria-label="Close search" className="px-2 text-gray-400 hover:text-gray-700">
                  <IconClose />
                </button>
              </div>
            ) : (
              <button type="button" onClick={() => setSearchOpen(true)} className="p-2 rounded-lg text-gray-700 hover:text-black transition-colors outline-none focus-visible:ring-2 focus-visible:ring-black/40" aria-label={t.nav.search}>
                <IconSearch />
              </button>
            )}
          </div>
          ...
          <div className="ml-2 relative" ref={langRef}>
            <button type="button" onClick={() => { setOpenMenu(null); setLangOpen(!langOpen) }} aria-expanded={langOpen} aria-haspopup="true" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm text-gray-600 hover:text-black hover:bg-gray-100 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-black/40">
              <IconGlobe />
              <span>{localeLabels[locale]}</span>
            </button>
            <AnimatePresence>
              {langOpen && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden min-w-[140px] z-10">
                  {locales.map((loc) => (
                    <button key={loc} type="button" onClick={() => { setLocale(loc); setLangOpen(false) }} className={`block w-full text-left px-4 py-2.5 text-sm transition-colors ${locale === loc ? 'text-black bg-black/5 font-medium' : 'text-gray-700 hover:text-black hover:bg-gray-50'}`}>
                      {localeLabels[loc]}
                    </button>
                  ))}
                </motion.div>
        ...
        >
          <span className={`block w-5 h-[2px] rounded bg-gray-900 transition-all duration-300 origin-center ${mobileOpen ? 'rotate-45 translate-y-[7px]' : ''}`} />
          <span className={`block w-5 h-[2px] rounded bg-gray-900 transition-all duration-300 ${mobileOpen ? 'opacity-0' : ''}`} />
          <span className={`block w-5 h-[2px] rounded bg-gray-900 transition-all duration-300 origin-center ${mobileOpen ? '-rotate-45 -translate-y-[7px]' : ''}`} />
        </button>
      </nav>
              ...
              <div className="w-56 border-r border-gray-100 py-4 px-2 shrink-0">
                {productCategories.map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => { setProductCat(productCat === cat ? null : cat); setProductFam(null) }}
                      ...
                      return hasVariants ? (
                        ...
                        >
                          <span className="flex items-center justify-between gap-2">
                            {fam}
                            <IconChevronRight className={`w-3.5 h-3.5 text-gray-300 shrink-0 transition-transform duration-200 ${productFam === fam ? 'rotate-90' : ''}`} />
                          </span>
                        </button>
                      ) : (
                        <Link
                ...
                {productCat && productFam && productsData[productCat][productFam].length > 0 && (
                  ...
                  >
                    {productsData[productCat][productFam].map((v) => (
                      <Link
                        key={v}
                        href={v === 'Overview' ? `/products/${toSlug(productFam)}` : `/products/${toSlug(productFam)}/${toSlug(v)}`}
              ...
              <div className="w-56 border-r border-gray-100 py-4 px-2 shrink-0">
                {Object.keys(applicationsData).map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => { setAppCat(appCat === cat ? null : cat); setAppSub(null) }}
                    className={`block w-full text-left px-4 py-2.5 text-sm rounded-lg transition-colors ${
                      appCat === cat
                        ? 'bg-black/5 text-black font-medium'
                        : 'text-gray-900 hover:bg-gray-50 hover:text-black'
                    ...
                    {Object.keys(applicationsData[appCat]).map((sub) => {
                      ...
                      return hasDetails ? (
                        ...
                        >
                          <span className="flex items-center justify-between gap-2">
                            {sub}
                            <IconChevronRight className={`w-3.5 h-3.5 text-gray-300 shrink-0 transition-transform duration-200 ${appSub === sub ? 'rotate-90' : ''}`} />
                          </span>
                        </button>
                      ) : (
                        <Link
        ...
        {openMenu === 'design-resources' && (
          <DropdownWrapper>
            <div className="max-w-[1280px] mx-auto flex min-h-[300px]">
              {/* Col 1: Categories */}
              <div className="w-56 border-r border-gray-100 py-4 px-2 shrink-0">
                {Object.keys(designResourcesData).filter((c) => c !== 'Overview').map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    ...
                    {Object.keys(aboutData[aboutCat]).map((sub) => {
                      ...
                      return hasItems ? (
                        ...
                        >
                          <span className="flex items-center justify-between gap-2">
                            {sub}
                            <IconChevronRight className={`w-3.5 h-3.5 text-gray-300 shrink-0 transition-transform duration-200 ${aboutSub === sub ? 'rotate-90' : ''}`} />
                          </span>
                        </button>
                      ) : (
                        <a key={sub} href="#solutions" className="block text-sm text-gray-700 hover:bg-gray-50 hover:text-black px-4 py-2 rounded-lg transition-colors">{sub}</a>
                      )
                    })}
                  </motion.div>
                )}
                    ...
                    >
                      <div className="space-y-0">
                        {items.map((item: string) => (
                          <a key={item} href="#solutions" className="block text-sm text-gray-600 hover:text-black py-1.5 px-2 rounded-lg transition-colors">{item}</a>
                        ))}
                      </div>
                    </motion.div>
                  )
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden bg-white border-t border-gray-100 overflow-hidden">
            <div className="px-6 py-4 space-y-1">
              <MobileAccordion label={t.nav.products} expanded={mobileExpanded} setExpanded={setMobileExpanded} id="products">
                {productCategories.map((cat) => (
                          ...
                          <div key={fam} className="ml-2 mb-1">
                            ...
                            <div className="ml-3 space-y-0.5">
                              {variants.map((v) => (
                                <Link
                                  key={v}
                                  href={v === 'Overview' ? `/products/${toSlug(fam)}` : `/products/${toSlug(fam)}/${toSlug(v)}`}
              ...
              <MobileAccordion label={t.nav.applications} expanded={mobileExpanded} setExpanded={setMobileExpanded} id="applications">
                {Object.keys(applicationsData).map((cat) => (
                  <div key={cat} className="mt-2">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{cat}</p>
              ...
              <MobileAccordion label={t.nav.designResources} expanded={mobileExpanded} setExpanded={setMobileExpanded} id="design-resources">
                {Object.keys(designResourcesData).filter((c) => c !== 'Overview').map((cat) => {
                  ...
                  return (
                    <div key={cat} className="mt-2">
                      ...
                      {items.length > 0 && (
                        <div className="ml-3 space-y-0.5">
                          {items.map((item) => (
                            <Link key={item} href={`/design-resources?q=${encodeURIComponent(item)}`} className="text-sm text-gray-500 hover:text-black py-0.5 block" onClick={() => setMobileOpen(false)}>
                              {item}
                            </Link>
                          ))}
                        </div>
              ...
              <MobileAccordion label={t.nav.aboutUs} expanded={mobileExpanded} setExpanded={setMobileExpanded} id="about">
                {Object.keys(aboutData).map((cat) => {
                  const subKeys = Object.keys(aboutData[cat])
                  return (
                    <div key={cat} className="mt-2">
                          ...
                          {subKeys.map((sub) => {
                            ...
                            return (
                              <div key={sub} className="py-0.5">
                                <a href="#solutions" className="text-xs text-gray-600 hover:text-black block">{sub}</a>
                                {items.length > 0 && (
                                  <div className="pl-3">
                                    {items.map((v: string) => <a key={v} href="#solutions" className="block text-xs text-gray-400 hover:text-black py-0.5">{v}</a>)}
                                  </div>
                                )}
                              </div>
                            )
              ...
              <div className="pt-3 border-t border-gray-100">
                <div className="flex flex-wrap gap-2 px-2">
                  {locales.map((loc) => (
                    <button key={loc} type="button" onClick={() => setLocale(loc)} aria-pressed={locale === loc} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${locale === loc ? 'bg-black text-white' : 'bg-gray-100 text-gray-600 hover:text-black'}`}>
                      {localeLabels[loc]}
                    </button>
                  ))}
                </div>
              ...
              <a href="#contact" onClick={() => setMobileOpen(false)} className="block text-center px-4 py-3 bg-black text-white rounded-lg font-medium hover:bg-gray-800 mt-3">
                {t.footer.contactUs}
              </a>
            </div>
          </motion.div>
